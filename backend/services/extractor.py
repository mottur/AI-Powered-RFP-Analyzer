...
verbose = False         # Use this verbose for logging from this module only


def chunk_text(pdf_bytes: bytes) -> list:
    """
    Chunks the raw bytes text.
    """
    ...
    for i, line_data in enumerate(all_lines):
        ...
        if re.match(r'^(?:[IVXLCDM]+|\d+|[A-Z])[.)]\s*$', line_text):
            ...
            if verbose:
                logger.info(f"  -> Found heading ID: '{pending_heading_id}'")
            continue

        # Check if there is a pending heading ID
        if pending_heading_id and line_text.isupper() and len(line_text.split()) >= 2:
            complete_heading = pending_heading_id + " " + line_text
            if verbose:
                logger.info(f"  -> Combined multi-line heading: '{complete_heading}'")
            
            is_heading = _is_heading(complete_heading, prev_line_text, next_line_text)
        ...
        if pending_heading_id:
            if verbose:
                logger.info(f"  -> Adding pending ID '{pending_heading_id}' to current chunk")
            if current_chunk:
                current_chunk["body"] += " " + pending_heading_id
            pending_heading_id = None

        # Normal line processing
        is_heading = _is_heading(line_text, prev_line_text, next_line_text)
...
def _is_heading(line_text, previous_line=None, next_line=None) -> bool:
    """
    Detects if the provided line is a heading or not.
    """
    line_text_clean = line_text.strip()
    if line_text_clean.startswith("o "):
        line_text_clean = line_text_clean.replace("o ", "\u2022")
    
    # DEBUG: Change pattern to print relevant line, otherwise no output is printed
    pattern = "blahblahblah"
    if pattern in line_text_clean and verbose:
        logger.info(f"\n=== CHECKING HEADING: '{line_text_clean}' ===")
        if previous_line:
            logger.info(f"  Previous: '{previous_line.strip()}'")
        if next_line:
            logger.info(f"  Next: '{next_line.strip()}'")

    # Reject empty or very short lines
    if len(line_text_clean) < 3:
        if pattern in line_text_clean and verbose:
            logger.info("  -> REJECT: Too short")
        return False
    ...
    if any(false_heading in line_text_clean.lower() for false_heading in common_false_headings):
        if pattern in line_text_clean and verbose:
            logger.info("  -> REJECT: Common false heading")
        return False
    ...
    if numbered_match:
        if pattern in line_text_clean and verbose:
            logger.info(f"  -> NUMBERED PATTERN FOUND: {numbered_match.groups()}")
        ...
        if pattern in line_text_clean and verbose:
            logger.info(f"  All numbered patterns in line: {all_numbered_patterns}")
        
        if len(all_numbered_patterns) > 1:
            if _are_consecutive_items(all_numbered_patterns[0], all_numbered_patterns[1]):
                if pattern in line_text_clean and verbose:
                    logger.info("  -> REJECT: Inline list detected (multiple patterns in one line)")
                return False
        ...
        if previous_line:
            ...
            if prev_match:
                ...
                if _are_consecutive_items(prev_id, current_id):
                    if pattern in line_text_clean and verbose:
                        logger.info(f"  -> REJECT: Continues list from previous line ({prev_id} -> {current_id})")
                    return False
            ...
            if next_line:
                ...
                if next_match:
                    ...
                    if _are_consecutive_items(current_id, next_id):
                        if pattern in line_text_clean and verbose:
                            logger.info(f"  -> REJECT: Starts list continued on next line ({current_id} -> {next_id})")
                        return False
        
        if pattern in line_text_clean and verbose:
            logger.info("  -> ACCEPT: Numbered heading")
        return True
    ...
    if pattern in line_text_clean and verbose:
        logger.info(f"  Uppercase: {is_uppercase}, Word count: {word_count}")
    
    # Accept uppercase headings
    if is_uppercase and 2 <= word_count <= 10:
        if pattern in line_text_clean and verbose:
            logger.info("  -> ACCEPT: Uppercase heading")
        return True

    if pattern in line_text_clean and verbose:
        logger.info("  -> REJECT: No heading pattern matched")
    return False

def _are_consecutive_items(prev_id, current_id):
    """
    Check if two heading IDs are consecutive (e.g. 1., 2.).
    """
...
def _extract_heading_id_and_title(full_text: str) -> tuple:
    """
    Extracts the heading id and title given the full heading
    """
    # Method 1: Match the pattern to separate heading id and title
    match = re.match(NUMBERED_PATTERN, full_text)
    if match:
        # Retrieve the heading id matching the pattern
        ...
        # Get the character right after the ID in the original text (delimiter)
